...
class KeywordsScreen(Screen):
    ...
    def on_mount(self):
        ...
        def trigger_refresh():
            if self.app:
                # Schedule background rescan instead of blocking UI
                self.app.call_later(self._start_scan_background)
        ...
        try:
            if self.new_folder_path and _os.path.isdir(self.new_folder_path):
                self._observer_new, self._stop_new = start_observer(
                    self.new_folder_path, trigger_refresh)
        except Exception:
            self._observer_new = None
            self._stop_new = None
        try:
            if self.old_folder_path and _os.path.isdir(self.old_folder_path):
                self._observer_old, self._stop_old = start_observer(
                    self.old_folder_path, trigger_refresh)
        except Exception:
            self._observer_old = None
            self._stop_old = None
    ...
    def _set_status(self, text: str) -> None:
        try:
            ...
            # Hide or show the status row to avoid blank vertical gap
            try:
                row = self.query_one('#kw-status-row')
